...
void CalendarState::refresh_today_date() {
  time_t raw;
  time(&raw);
...
void CalendarState::update_view_date(struct tm day_in_view_month) {
  struct tm t;
  memset(&t, 0, sizeof t);
  t.tm_mday = 1;
...
void CalendarState::set_view_prev_month() {
  first_day_of_view_month.tm_mon--;
  mktime(&first_day_of_view_month);
}

void CalendarState::set_view_next_month() {
  first_day_of_view_month.tm_mon++;
  mktime(&first_day_of_view_month);
}
